...
const Orders = () => {
    ...
    try {
      ...
      const allOrders = users.reduce((acc, user) => {
        if (user.orders && Array.isArray(user.orders)) {
          return acc.concat(user.orders.map(order => ({
            ...order,
  ...
  const updateOrderStatus = async (orderId, newStatus, userId) => {
    ...
    try {
      ...
      try {
        ...
        if (localStorageUser) {
          ...
          if (parsedUser.id === userId) {
            const updatedLocalOrders = parsedUser.orders.map(order => 
              order.orderId === orderId 
                ? { ...order, status: newStatus }
                : order
            );
            
            parsedUser.orders = updatedLocalOrders;
            localStorage.setItem('user', JSON.stringify(parsedUser));
            console.log('✅ LocalStorage updated for user:', userId);
          }
        }
      ...
      console.log(`✅ Order ${orderId} status updated to ${newStatus} in both DB and localStorage`);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-4 border-[#2eb4ac] border-t-transparent mx-auto"></div>
